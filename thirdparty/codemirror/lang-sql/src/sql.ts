...
export class SQLDialect {
  ...
  static define(spec: SQLDialectSpec) {
    let d = dialect(spec, spec.keywords, spec.types,spec.functions)
...
export const MySQL = SQLDialect.define({
  ...
  specialVar: "@?",
  identifierQuotes: "`",
  functions: "if(test,t,f) ifnull(test,val) nullif(test,val) abs(x) bin(x) ceiling(x) exp(x) floor(x) greatest(x1,x2,...,xn) least(x1,x2,...,xn) ln(x) log(x,y) mod(x,y) pi(x) rand(x,y) round(x) sign(x) sqrt(x) truncate(x) avg(x) count(col) min(col) max(col) sum(col) group_concat(col) ascii(char) bit_length(str) concat(s1,s2...,sn) concat_ws(sep,s1,s2...,sn) find_in_set(str,strs) insert(str,x,y,instr) left(str,x) length(str) ltrim(str) position(substr,str) quote(str) repeat(str,old,new) reverse(str) right(str,x) rtrim(str) strcmp(s1,s2) trim(str) ucase(str) upper(str) date(str) curdate() current_date() curtime() current_time() date_add(date,INTERVAL-int-keyword) date_format(date,fmt) time_format(time,fmt) date_sub(date,INTERVAL-int-keyword) dayofweek(date) dayofmonth(date) dayofyear(date) dayname(date) from_unixtime(ts,fmt) hour(time) minute(time) mouth(date) monthname(date) now() quarter(date) week(date) year(date) utc_date() utc_time() utc_timestamp() aes_encrypt(str,key) aes_decrypt(str,key) decode(str,key) encrypt(str,salt) encode(str,key) md5(str) password(str) sha(str) format(x,y) inet_aton(ip) inet_nota(num) convert(val,type) database() version() connection_id() found_rows() user() json_array(x1,x2,...,xn) json_object(k1,v1,k2,v2...,kn,vn) json_quote(json) json_contains(json,val,path1,path2,...,pathn) json_contains_path(json,one_or_all,path1,path2,...,pathn) json_extract(json,path1,path2,...,pathn) json_keys(json,path1,path2,...,pathn) json_search(json,one_or_all,search) json_array_append(json,path,x1,x2,...,xn) json_append(json,path,x1,x2,...,xn) json_array_insert(json,path1,val1,path2,val2,...,pathn,valn) json_insert(json,path1,val1,path2,val2,...,pathn,valn) json_replace(json,path1,val1,path2,val2,...,pathn,valn) json_set(json,path1,val1,path2,val2,...,pathn,valn) json_merge(json1,json2,...,jsonn) json_remove(json,path1,path2,...,pathn) json_unquote(json) json_depth(json) json_length(json,path) json_valid(json)",
  keywords: "explain in group-by is-null is-not-null left-join right-join inner-join natural-join on order-by select where from having and as or like between not is limit distinct union all set update insert-into values asc desc offset group order by join left right inner natural insert into delete drop alter create table case when then if else end begin commit rollback",
  types: "json tinyint smallint mediumint int integer bigint float double decimal date time datetime year timestamp char varchar tinyblob tinytext blob text mediumblob mediumtext longblob longtext",
})

/// The standard SQL dialect.
export const StandardSQL = MySQL
